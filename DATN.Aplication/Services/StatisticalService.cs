    ...
    public class StatisticalService: IStatisticalService
	{
		...
		public async Task<ResponseData<Statistical>> StatisticalIndex(int type = 1)
		{
			...
			try
			{
				...
				cusStatistical.Label = new List<string>();
				cusStatistical.Amount = new List<double>();
				if (type==1)
                {
					...
					for (int i = 7; i <= timeNow; i++)
					{
						...
						cusStatistical.Label.Add(i + "h");
						if (bookingInHour!=null&& bookingInHour.Any())
                        {
							cusStatistical.Amount.Add(bookingInHour.Count());
						}
						cusStatistical.Amount.Add(0);
				...
				else if (type == 2)
				{
					...
					for (var date = startDate; date <= today.Date; date = date.AddDays(1))
					{
						...
						cusStatistical.Label.Add(date.ToString("dd/MM"));
						if (bookingsInDay != null && bookingsInDay.Any())
						{
							cusStatistical.Amount.Add(bookingsInDay.Count());
						}
						else
						{
							cusStatistical.Amount.Add(0);
				...
				else if (type == 3)
				{
					...
					{
						var bookingsInDay = lstBooking.Where(p => p.BookingTime.Date == date);
						cusStatistical.Label.Add(date.ToString("dd"));
						if (bookingsInDay != null && bookingsInDay.Any())
						{
							cusStatistical.Amount.Add(bookingsInDay.Count());
						}
